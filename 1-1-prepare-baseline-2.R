...
for (dir in dirzip) {
  ...
  # for(i in (12L:(N_PART - 1L))) {
    ...
    views_interactions = views_data[baseline_1_data$dt, .(
      # row index
      i, 
      # column indices
    ...
    rm(views_interactions)
    # cbind with events matrix and convert to CSR
    m = cbind(baseline_1_data$X, m_views) %>% as("RsparseMatrix")
    save_rds_compressed(list(X = m, y = baseline_1_data$y), file = sprintf("%s/%03d.rds", b2, i))
